...
> both reference screenshots, evidence). Confirmed and **fixed the one P0/P1 functional defect**
> in that bucket: dead Play/Save on heart-series articles (PC-002). Pushed
> `lane/premiumcontrols-heart-series-wiring-2026-06-26` to source. Evidence:
> `evidence/S3-PC002-fix-browser-witness.txt`.

**PC-002 — heart-series Play/Save dead → FIXED**
- Cause: `KrajneBody.astro` / `Rimlyanam7Body.astro` had `gb-ember`+`gb-save` in `.gbs2-rfoot` but no `data-fc-root` → controller's `closest('[data-fc-root],[data-fc-controls]')` guard + main-init loop skipped them.
- Fix: added `data-fc-root data-fc-mode="series-rich" data-fc-variant="heart"` to `.gbs2-rfoot` (mirrors working Nagornaya). 2-line change.
- **Browser witness (Playwright, production-like dist):** both pages `root._gbClusterInit=true`, `window.__gbCluster=object`, Save `aria-pressed` false→true, Play opens speed panel, 0 console errors.

**Re-confirmed while doing this (PC-003 = my S3-N4):** first build attempt 404'd all assets — build-mode trap (need `strangler:build:production-like`) + stale hardcoded `?v=efd81d3a` in `src/*.astro` rescued only by `astro-cache-bust-postbuild.js` (527 replacements, drift→0). The cache-bust-ignores-`src/*.astro` engine is still open → dedicated system lane.

**Other PremiumControls findings (assessed, NOT closed — separate lanes per verifier's feature-completion proposal):**
| ID | Severity | My assessment |
|---|---|---|
| PC-001 | P1 | Real architecture gap (no `PremiumControlAnchor`; controls self-position via `fixed`). Needs design lane; do not mix with PC-002. |
| PC-003 | P1 | = my S3-N4. Re-confirmed. Cache-bust must cover `src/*.astro`. |
| PC-004 | P1 | Real — CSS duplicated between component `<style is:global>` and `css/floating-cluster.css`. Owner wants feature-first, then CSS cleanup. |
| PC-005 | P2 | PlayEmber split semantics + storage key `gbx-tts-rate` vs plan `gb:audio:rate`. Needs decision before site-wide rollout. |
| PC-006 | P2 | No route-archetype rollout audit. Governance lane. |

**Recommendation:** PC-002 is the only one that was a *visible broken control*, so I fixed it first (matches owner's "finish feature topово" intent). PC-001/003/004/005/006 are architecture/governance and should land as the focused `premiumcontrols-feature-completion` system lane the verifier proposed, BEFORE broad CSS/JS cleanup.
