...
- ~~**Остаток:** у `/articles/dzhon-gill-chast-1-chelovek/` `og:image` пустой~~ — **ОТОЗВАНО в Wave 11:** это ложное срабатывание grep (у страниц Гилла порядок атрибутов `content=… property=…`). `og:image` = `https://gospod-bog.ru/images/og-dzhon-gill-chast-1-chelovek.webp` присутствует. Зеркала вроде `www.` / github.io дадут пустые обложки: `UNPROVEN`.
...
# Wave 11 — мета-данные и структура документа (статически, 104 страницы)

Песочница сбросилась в четвёртый раз, продукт пересобран на `d0e04a9c` (гейты зелёные). Скрипт `evidence/meta-structure.py` разбирает `<meta>`/`<link>` без зависимости от порядка атрибутов. Данные: `evidence/meta-structure-104.json`.

## Коррекция собственной ошибки

- Первая версия скрипта (регулярка `name="…" content="…"`) показала у 7 страниц (Гилл ч.1–4, исторический контекст, справочник, «Герменевтика») отсутствие description, canonical и og:*. Это **ложное срабатывание**: на этих страницах атрибуты идут в порядке `content="…" property="…"`. Скрипт переписан, повторный прогон — 0.
- Тем же дефектом grep было вызвано утверждение Wave 8 «у Гилла ч.1 пустой og:image». Оно **отозвано** и зачёркнуто в разделе Wave 8.

## Результат — чисто

| проверка | результат |
|---|---|
| `<title>` отсутствует | 0 / 104 |
| description / canonical / og:title / og:image отсутствуют (индексируемые, 94) | 0 |
| canonical не на себя | 0 |
| дубли title / description | 0 |
| `html[lang]` ≠ `ru` | 0 |
| `<img>` без `alt` | 0 |
| h1 ≠ 1 | 1 — `/konfessii/russkij-baptizm/_app/` (`noindex`, вне sitemap, h1 рендерится JS в рантайме — Wave 8 видел `H1.mb-7`). Не находка. |

## Observation — длина сниппетов

- `title` > 70 символов: 44 индексируемые страницы (длинные заголовки статей с подзаголовком через «—» + суффикс сайта). `description` > 200: 13 (в основном цикл «Сердце»). Поисковики обрежут. Решение контентное, triage only.

## A11Y-10 — пропуски уровней заголовков — `candidate`, low

- Гилл ч.1–4: `h1 → h4`, карточки серии `.gill-card__title` идут сразу после h1 без промежуточного h2 («Человек», «Учёный», …).
- «Код да Винчи»: `h2 → h4` ×6, оборот flip-карточек `.flip-card-back` («Миф романа …»).
