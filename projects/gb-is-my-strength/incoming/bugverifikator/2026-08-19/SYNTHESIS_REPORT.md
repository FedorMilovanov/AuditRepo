...
|---|---|---|---|---|
| bugverifikator (`incoming/bugverifikator/2026-07-17/`) | cb3681e | Runtime, SW, Astro Components | source | AR-IDX-JS-02-MULTIWRITER, SW-PWA-FRESHNESS, MISSING-BUTTON-TYPE, SEARCH-LAZY-LOADER-DRIFT |
...
| G. Runtime Ownership | AR-IDX-JS-02-MULTIWRITER | single theme writer | remove legacy writes from `enhancements.js`/`site.js` | source census | yes |
| H. HTML Hygiene | MISSING-BUTTON-TYPE + SEARCH-LAZY-LOADER-DRIFT | robust button types / consistent loader | add `type="button"`; unify `BaseLayout` search loader | source audit | no |
| I. PWA Freshness | SW-PWA-FRESHNESS | revisioned precache | add `?v=` to runtime assets in `sw.js` `PRECACHE_ASSETS` | cache-bust test | no |
...
## 6. Admitted by bugverifikator (2026-08-19 wave)

| Finding | Result | Decisive evidence | Owner / next |
|---|---|---|---|
| `AR-IDX-JS-02-MULTIWRITER` | admit as residual | `js/enhancements.js` L536 `localStorage.setItem('theme', ...)` + `site.js` variants write to legacy key; conflicts with `reader-preferences.js` canonical owner | reader preferences owner; remove legacy writes from old runtime |
| `MISSING-BUTTON-TYPE` | admit as residual | `HardTextsPageChrome.astro` L54, L58, L121 `<button>` elements missing `type="button"` | astro layout owner; add explicit types |
| `SEARCH-LAZY-LOADER-DRIFT` | admit as residual | `BaseLayout.astro` L199 `o&&window.GBSearch&&window.GBSearch.open&&window.GBSearch.open()` drift vs canonical `if` check elsewhere | search engine owner; unify loader snippet |
| `SW-PWA-FRESHNESS` | admit as improvement | `sw.js` `PRECACHE_ASSETS` includes `/js/reader-preferences.js` etc without `?v=` revisioning; blocks updates under `cacheFirst` | service worker owner; add revision tracking to precache |
