# REPORT — Генеалогия Спасителя: фундаментальный аудит и стратегия введения
...
Референсы владельца — GPT-изображения «Генеалогия Спасителя» (2 новых скрина в
...
4. **Платформа (рекомендация):** собственный лёгкий SVG-движок **GenealogyEngine**
...
6. **Где и как вводить:** маршрут остаётся `/rodosloviye/` (URL-капитал: sitemap,
   search-manifest, baseline), бренд страницы — «Генеалогия Спасителя»; режим
   `strict-native-app` (уже назначен); работа только в lane (`lane/genealogy-*`,
...
7. **План — 6 фаз (месяцы, без дедлайнов):** 0 Решения → 1 Фундамент данных →
   ...
   релиз с discoverability. Acceptance — 8 эталонного уровня критериев (§S8).
...
- Прецедент стратегии: `incoming/arena-agent-karty-strategy/2026-07-07/` (STRATEGY.md:
  4 закона, 6 фаз, эталонного уровня критерии, ANTI-PATTERNS A1–A15). Генеалогия обязана
...
**B (рекомендуется): GenealogyEngine — собственное SVG-ядро.**
...
1. **Маршрут:** `/rodosloviye/` (сохранить; бренд H1 «Генеалогия Спасителя»,
...
3. **Процесс:** только lane-ветки `lane/genealogy-<phase>`; Risk 2–3 (route
...
7. **Discoverability (закрыть GEN-ORPHAN-01):** карточка-блок на главной (dashboard),
   ...
   Включать ссылки ТОЛЬКО при достижении эталонного уровня (§S8) — до того маршрут живёт
...
| **5. QA + релиз** (2–4 нед) | visual-baseline, interactive-audit сценарии, a11y-аудит, перф-замеры на устройствах, THEN discoverability-ссылки | релизный lane + скрины базлайна | 8/8 эталонного уровня (§S8); ссылки включены; 14-дневный freeze наблюдения |
...
## S8. Критерии эталонного уровня (acceptance gate, аналог karty STRAT-04)
...
| Морфинг кластеров — сложнейшая часть рендера | Средняя | Phase 2 прототип ДО обязательств; fallback: жёсткое переключение уровней без морфинга (всё ещё эталонного уровня minus) |
...
5. **Бренд**: РЕШЕНО владельцем 2026-07-11 — «Генеалогия Спасителя» / «От Адама до Христа Спасителя» на `/rodosloviye/` (НЕ «атлас»). URL не меняем.
...
- Suggested repair lane: lane/genealogy-* (Phase 1-3, §S7)
...
- Suggested repair: Phase 5 (§S6.7) — ссылки ПОСЛЕ эталонного уровня; не чинить сейчас.
...
- Bug IDs: GEN-STRAT-01, GEN-SCALE-01, GEN-UX-01, GEN-CODE-01 → lane/genealogy-*
