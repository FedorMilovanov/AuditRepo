...
| `SEARCH-P3-01` | yes — trigger label inventory across built routes and `search.js` injection strings | yes — `PASS6_POLISH_DISCOVERY_PROBE.json` | no — browser not required for source label inconsistency; needed for final platform-adaptive behavior | yes — polish row, no duplicate found | confirmed-current | One shared/platform-aware search trigger label contract. |
| `SEARCH-P3-02` | yes — `results.slice(0,10)` / `slice(0,12)` in `js/search.js` | yes — `PASS6_POLISH_DISCOVERY_PROBE.json` raw Pagefind counts exceed visible caps | no — browser needed for final show-more UX closure | yes — discovery-depth row, no duplicate found | confirmed-current | Show raw total or add incremental show-more. |
| `SEARCH-P3-03` | yes — copy URL construction and label string in `js/search.js` | yes — `PASS6_POLISH_DISCOVERY_PROBE.json` | no — browser not required for source mismatch; needed for final copy UX closure | yes — polish row, no duplicate found | confirmed-current | Align canonical/current-origin copy behavior and label. |
