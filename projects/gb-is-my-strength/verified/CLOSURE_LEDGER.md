...
## 2026-09-12 — audit-harness authority / portability false-signal closure wave

- Scope: four independent audit-harness roots discovered while re-verifying current Product authority. None is admitted as a new active Product defect: each root was bounded to evidence-generation false-red/false-green behavior and is now repaired and Product-merged.
- System theme: `ST-AUDIT-HARNESS` — validation/release evidence must fail closed and reflect current source/artifact authority rather than incidental local state or implementation spelling.
- Product reconciliation anchor: current Product `main` `65b56da4cb9f0ee9a0d3c843b443574b3ea5a380`.
- Result:
  - closed-by-system-fix: **validator authority drift** — Product #2002 (`fix(audit): align validators with current authority`), exact certified head `4a4bd59c07009b1107d74b9dab9b4925872ddfe4`, merge `6fa9e564286f450c5cab0d558c3a6f9d00507220`. MDX structure audit no longer treats frontmatter as reader body; reference-only legacy HTML no longer competes with current semantic title/OG authority; current 1199/1200 layout contract is accepted rather than a stale breakpoint allowlist.
  - closed-by-system-fix: **visual-parity stale-dist authority drift** — Product #2006 (`fix(audit): separate source and dist visual parity`), exact certified head `7c1d17f181ae808b7ab201c3a78cd2551dd3b925`, merge `971bfcd61065326b67f95974de268e1b04bb383e`. Source audits no longer silently change semantics because an unrelated local `dist/` happens to exist; strict artifact checks require explicit `--require-dist`; the production contract invokes strict route audits. Articles parity now proves the exact projected catalogue route set (80/80) rather than a stale thumbnail-count heuristic.
  - closed-by-system-fix: **Windows npm child-launch portability / fail-open status handling** — Product #2012 (`fix(audit): make npm child launches portable`), exact certified head `1ae897b6a0a96d04fc7d2745793b96ee102f377f`, merge `33a958c2bf47447e06ba09d92552ab98df15837c`. Direct `spawnSync('npm.cmd', ...)` / `execFileSync('npm.cmd', ...)` on the audited Windows environment produced `EINVAL` with `status=null`; `build-pagefind.js` could convert that launch failure into exit 0 through `status || 0`. All six callers now use one fail-closed portable npm launcher; launch errors/non-numeric status cannot become success, Windows shell metacharacters are rejected, and the repository-wide contract forbids bypasses.
  - closed-by-system-fix: **implementation-coupled map-init source oracle** — Product issue #2013 / PR #2016 (`fix(audit): make map init guard semantic`), exact certified head `e572fc6c8cd28dd365b2eef496a910118b79bb48`, merge `65b56da4cb9f0ee9a0d3c843b443574b3ea5a380`. The Ishod audit had false-red coupling to local variable name `inst` and exact error text although current runtime used `instance` with the same fail-closed guard; Avraam carried the same latent oracle risk. The shared semantic contract now proves engine guard ordering, captures the actual `createMap()` result identifier, and requires its null guard before ready state; rename mutations pass while removed/misordered/wrong-variable guards fail.
- Exact-head regression witnesses:
  - #2002 head: Metadata/IndexNow `34682824908`, Shared Files `34682824851`, Metadata SSOT `34682824824`, Source Authority `34682824856`, Deploy Candidate `34682824834` — all SUCCESS.
  - #2006 head: Shared Files `34690390792`, Metadata SSOT `34690359841`, Source Authority `34690359832`, Visual Parity pixel-diff `34690359821`, Metadata/IndexNow `34690359833`, Deploy Candidate `34690359830` — all SUCCESS; the earlier Shared Files run `34690359842` was superseded/cancelled by the later head metadata event and is not closure authority.
  - #2012 head: Node Toolchain `34697164416`, Shared Files `34697162765`, Native Source `34697162664`, Source Authority `34697162723`, Deploy Candidate `34697162696`, Metadata/IndexNow `34697162714`, Editorial Metadata v3 `34697162777`, Metadata SSOT `34697162704` — all SUCCESS.
  - #2016 head: Metadata/IndexNow `34705972402`, Shared Files `34705972404`, Metadata SSOT `34705972419`, Source Authority `34705972408`, Deploy Candidate `34705972407` — all SUCCESS, including the final Gill Chromium/browser tail.
- Independent local/adversarial witnesses retained in the detailed receipt:
  - corrupt stale article `dist`: default source audit remains PASS while strict `--require-dist` fails; restored fresh candidate proves exact projected routes 80/80;
  - Windows direct npm launch reproduces `EINVAL/status=null`; canonical helper executes pinned npm 10.9.8 and Pagefind 1.5.2 indexes 92 pages / 25,932 words with exit 0; `astro:audit:article-mdx:strict` completes 35 routes;
  - map-init mutation matrix accepts `inst → instance → mapResult` rename and rejects missing guard, guard after ready, engine guard after `createMap()`, and wrong-variable guard; Avraam audit 55/55 and Ishod no-build audit PASS;
  - after the npm-launch and map-oracle repairs, `validate:static-publication:light` completes EXIT=0 end-to-end.
