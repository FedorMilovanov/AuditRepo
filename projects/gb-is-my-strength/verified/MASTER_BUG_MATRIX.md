...
Current Strangler hidden-verifier recheck: [`../verification/2026-08-08-strangler-self-verifier-hidden-blocker/REPORT.md`](../verification/2026-08-08-strangler-self-verifier-hidden-blocker/REPORT.md).  
...
| `SYS-STRANGLER-RETIREMENT` | Safely retire/quarantine legacy reference storage only after logical identity, dependency ownership, inventory/integrity/parity and post-move verification are all fail-closed. | Product `#1222@22983986fadc50f22fb831a2b956915576448aad` is clean/narrow (`behind=0`, five intended files) and now **7/7 exact-head workflows are SUCCESS**, but this does **not** authorize merge: fresh audit proves `scripts/legacy-shadow-retirement-readiness.mjs` still reads governed legacy bytes from current physical paths while its own ledger row classifies the dependency as nonblocking. The reported `21 blockers` is therefore incomplete as a pre-move storage-awareness inventory and can eventually permit a false `physicalMoveAuthorized`. Keep Wave A narrow by truthfully reclassifying that verifier as `must-update-before-move` (preferred), or explicitly widen the wave to migrate the verifier through `migration/legacy-reference-path.js` plus quarantine/ambiguity/absence/hash fixtures. Rerun exact-head gates and update blocker arithmetic/PR record before merge. [`REPORT`](../verification/2026-08-08-strangler-self-verifier-hidden-blocker/REPORT.md) |
| `SYS-SOURCE-AUTHORITY-TRIGGER-CLOSURE` | Make Source Authority workflow applicability fail closed against the publication source surfaces its static-publication validation actually consumes. | Product issue `#1244` remains the canonical guard-health owner. Merged `#1245` closed the concrete Baptist false-negative by adding `src/content/articles/**` and `src/components/baptisty-rossii/**` to PR/push filters, so that witness must not remain a direct defect. But `#1245` intentionally changed only four filter lines and added no adversarial/path-applicability contract. Remaining bounded DoD: inventory protected roots consumed by the gate, assert PR+push applicability and mutation-test removal of representative roots so a future validator-scope expansion cannot silently recreate the false-negative class. [`REPORT`](../verification/2026-08-08-total-current-gold-audit/REPORT.md) |
...
- `#1222` owns Strangler storage abstraction Wave A at `22983986...`. It is `behind=0`, five-file scoped and **7/7 exact-head green**, but remains **not merge-authorized** because the retirement-readiness verifier itself is an uncounted physical-storage dependency. Repair ledger/storage truth first; green CI alone is insufficient. [`REPORT`](../verification/2026-08-08-strangler-self-verifier-hidden-blocker/REPORT.md)
