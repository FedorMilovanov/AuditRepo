...
## Статистика (обновлено 2026-07-14 live-reverify @ `2ca2af3b`, 3 прохода: +1 закрыто source [CI-INDEXNOW @ 3a43cada] + AR-CI-RED; +6 deploy-блокеров; +3-й проход deep CSS/JS: +1 P1 AUDIT-CSS-SITECSS-STRUCT-CORRUPTION + 2 P3 [AUDIT-CSS-NO-STRUCTURAL-PARSE, AUDIT-CSS-DEAD-KEYFRAMES-TOKENS])
...
| Рефакторинг | 4 |
| AuditRepo | 3 |
| **Всего открыто (матрица)** | **53** |
| Системный бэклог вне матрицы | см. `SUPER_AUDIT_2026-07-06_14a49be8.md` (волны W1–W10) |
| False positives отклонено | 5 |
| Passes processed | 99+ (live-reverify 2026-07-14 @ 2ca2af3b ×4 прохода [актуализация + deploy-блокеры + deep CSS/JS + detailed CSS/JS], arena-auditor-meta-governance) |
...
- **2026-07-14 (проход 4) — Детальный CSS/JS-аудит, source `2ca2af3b`, Node v22.22.3.** Per-file AST-скан
  ВСЕХ CSS (не только site.css): нашёл **2-й файл со структурной порчей** — `floating-cluster.css`
  (комментарии 237`/*`/236`*/`; голый текст «v15: MOBILE PREVIEW» поглощает правило `.mobile-bottom-bar`;
  импакт измерен как мелкий — теряются лишь justify-content/font, остальное переопределяется позже) →
  **AUDIT-CSS-FLOATCLUSTER-COMMENT-CORRUPTION (P3)**. JS детально: таймеры сбалансированы (нет утечек),
  все localStorage-записи (28) и JSON.parse (2 «голых») обёрнуты в try/catch/.catch — **нового бага нет**
  (проверено, не переоценено); ReDoS нет; search.js 22/0 слушателей — на singleton-элементе, не утечка.
  Бюджет-реверифай: D-3 обновлён (JS 469101, было 375041; **CSS 554013 > 425000 — больше не «в норме»**),
  NEW-CSS-BUDGET-01 получил конкретные цифры. Source НЕ трогал. Evidence: `incoming/.../evidence/
  css-js-deep-audit-pass4-2026-07-14.txt`; разбор — reverify §CSS/JS pass 4.
- **2026-07-14 (проход 3) — Глубокий CSS/JS-аудит (по запросу владельца), source `2ca2af3b`, Node v22.22.3.**
  Инструменты: `postcss@8.5.16` + `css-tree` (оба уже в node_modules), `acorn`, `node --check`.
  **CSS:** оба парсера падают на `css/site.css` (postcss 40:16304; css-tree 9 ошибок) — **структурная
  порча** из 5 сломанных правил (пустой `@media prefers-reduced-motion` → a11y-регрессия; висячие
  списки селекторов; `@supports` в середине списка; пустое `html.dark`; битый `:rgba()`-декл). Гейты
  слепы (audit-pro = только баланс скобок). Заведено **AUDIT-CSS-SITECSS-STRUCT-CORRUPTION (P1)** +
  **AUDIT-CSS-NO-STRUCTURAL-PARSE (P3)** (гейт-гэп) + **AUDIT-CSS-DEAD-KEYFRAMES-TOKENS (P3)** (дубль
  `@keyframes fx-breathe` + 33 мёртвых токена). Проверка var-ов: **0 ложных** (все «undefined» имеют
  fallback/JS-set/`@layer base`). **JS:** все 15 файлов чисты по `node --check`; 0 debugger/TODO;
  BUG-PERF-001 реверифицирован 349/25 (было 339/25, +10 — чуть хуже); XSS-поверхность защищена
  (`tt()`/strip) — нового XSS нет; реверифицированы still-open NEW-HIGHLIGHTS-NO-REINIT-GUARD и
  NF-VOSK-DEAD-SPLITSENTENCES. Source НЕ трогал. Evidence: `incoming/arena-auditor-meta-governance/
  2026-07-14/evidence/css-js-deep-audit-2026-07-14.txt`; разбор — reverify §CSS/JS deep-dive.
...
- **2026-07-14 — Live-reverify: актуализация устаревшего канона, source `b8459bdf` → `2ca2af3b` (deploy 🔴 RED).**
  ...
  **owner-gated**: source-репо НЕ трогал. Полный разбор: `reverify/CURRENT_HEAD_REVERIFY_2026-07-14_2ca2af3b.md`;
  ...
  - **2-й проход (глубже, реальный production-like билд):** `strangler:build:production-like` = OK (58 стр., dist),
    что дало **artifact-witness**: genealogy-build шаблоны дают **0 файлов в dist** → REG-VALIDATE-GENEALOGY-TEMPLATE
    подтверждён как **tooling-scope** (не served-дефект); фиксы A и C **доказаны локально** (skip walker → 0;
    `cache-bust --write` → 0, 82 файла). Найдено **ещё 3 deploy-блокера**: **GATE-CSS-IMPORTANT-RATCHET** (P2 —
    `site.css` 210 `!important` > 200; `css:layer:validate` тоже красный), **AUDIT-ATLAS-DOC-PATH-LEAK** (P3 — 2 новых
    atlas-дока с repo-путём), **AUDIT-FORBIDDEN-JS-NAGORNAYA** (P3 — allowlist-gap, файл РЕАЛЬНО используется 5
    nagornaya-страницами, НЕ удалять). Итого 6 классов блокеров (сводная таблица в reverify §2).
  - **🟣 AR-CI-RED (закрыт):** при синхронизации `origin/main` оказался на **581 коммит впереди** (параллельные
    агенты), но **CI AuditRepo был красный** — 3 нарушения структуры (stray root `DEBT-REGISTER.md`; intake без
    README/REPORT; невалидное имя intake-папки). Починено минимально/без удаления чужого контента (CLEANUP §7):
    `git mv` в `working/` + additive README ×2 + `git mv` папки в `-r1`. Оба валидатора PASS. Моя ветка
    ребейзнута на их main (их 581 коммит сохранён, мои правки поверх).
