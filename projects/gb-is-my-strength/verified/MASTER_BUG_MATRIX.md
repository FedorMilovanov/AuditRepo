...
| Поле | Значение |
|---|---|
| Source verification anchor | `f9d0120718569c510833dba7a3abd68ce2f6a003` (native Astro production-like dist inventory: `NG-DARK-01` = 19 historical-scope tokens / 443 uses without complete direct dark coverage; PR #151 legacy-shadow counts superseded; no Product mutation, browser, production or TTS claim). |
...
| ID | Описание | Коммит |
|---|---|---|
| NG-BODY-01 | ✅ **CURRENT DUPLICATE / MERGED INTO `NG-DARK-01`; NATIVE-DIST AUTHORITY CORRECTED 2026-08-04.** Product `f9d0120718569c510833dba7a3abd68ce2f6a003` passed the native Nagornaya parity contract and production-like build. Five chapter routes use dark-capable `bg-stone-900`, but built `/nagornaya/`, `/nagornaya/istochniki/` and `/nagornaya/nakhodki/` still use `bg-stone-100`; exact route-linked dist scanning found no direct dark-context rule for that token on those using routes. PR #150’s FIXED-CURRENT explanation is therefore superseded. This row remains in the closed section only to preserve Single-Writer-Per-Fact: the current body subset is owned exclusively by open `NG-DARK-01` and must be included in its browser/computed-style acceptance boundary. No Product mutation, browser, production or TTS claim. | `f9d01207` |
...
| NG-DARK-04 | ✅ **DUPLICATE / MERGED INTO `NG-DARK-01`; NATIVE-DIST AUTHORITY RECONCILED 2026-08-04.** Exact Product `f9d0120718569c510833dba7a3abd68ce2f6a003` passed the native route contract and production-like build. Built output contains 13 `bg-rose-50` uses and route-linked CSS covers the token on every using native route. The row remains closed because it has no independent repair owner; PR #151 legacy-shadow counts are superseded. No Product mutation, browser, production or TTS claim. | `f9d01207` native dist |
| NG-DARK-05 | ✅ **DUPLICATE / MERGED INTO `NG-DARK-01`; NATIVE-DIST AUTHORITY RECONCILED 2026-08-04.** Exact built native output at Product `f9d0120718569c510833dba7a3abd68ce2f6a003` retains 13 `bg-stone-100` uses without complete direct dark coverage, including three body owners, and 2 `bg-stone-200` uses in the same residual. The former PR #151 statement that current `bg-stone-100` is covered is superseded. This aggregate row remains closed so the current stone subset has one repair owner only: open `NG-DARK-01`. No Product mutation, browser, production or TTS claim. | `f9d01207` native dist |
...
| NG-MOBILE-01 | ✅ **AGGREGATE DUPLICATE / MERGED; CURRENT OWNER LINKS RECONCILED 2026-08-04.** This row contains no independent mobile root cause: the current native `bg-stone-100` body subset is represented by closed duplicate `NG-BODY-01` but owned exclusively by open root `NG-DARK-01`; chapter-specific TOC accent remains owned by open `NG-TOC-01`; inline hero height/adaptivity remains owned by open `NG-A11Y-01`. All three repair owners remain open. No Product mutation, browser, production or TTS claim. | `f9d01207` native dist |
...
| NG-DARK-01 | ⚠️ **CURRENT / NATIVE-DIST AUTHORITY CORRECTED 2026-08-04:** Product `f9d0120718569c510833dba7a3abd68ce2f6a003` passed the native Nagornaya parity contract and a production-like build. Exact built `dist/nagornaya/**/index.html` contains 20 currently used historical-scope utility tokens / 456 uses; linked built CSS covers 1 tokens / 13 uses (`bg-rose-50` (13×)). The authoritative source residual is **19 tokens / 443 uses** without complete direct dark coverage: `border-stone-100` (167×), `text-amber-600` (45×), `text-blue-600` (41×), `text-rose-600` (41×), `text-purple-600` (40×), `text-blue-700` (22×), `text-emerald-700` (15×), `text-emerald-600` (14×), `bg-stone-100` (13×), `text-purple-700` (12×), `text-amber-800` (11×), `text-amber-700` (8×), and 7 more. This supersedes PR #151 legacy-shadow counts. Missing direct coverage is not yet a browser contrast failure; verify computed native output before a bounded Product repair. No Product mutation, browser, production or TTS claim. | native production-like dist `f9d01207`; `reverify/CURRENT_HEAD_REVERIFY_2026-08-04_f9d01207_nagornaya-dark-native-dist.md` |
