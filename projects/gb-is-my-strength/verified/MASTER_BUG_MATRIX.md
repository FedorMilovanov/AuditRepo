...
| Deploy | 🔴 **RED** — `deploy.yml` падает на **«Static publication gates»** с 2026-07-11 (80 runs = 56 failure + 24 cancelled, 0 success после `29138555390`@`007b67de`). Последний failure: `29338523013`@`2ca2af3b`. |
...
|---|---|---|
| DEPLOY-STATIC-GATES-RED-2026-07-11 | 🔴 **Production застрял на `007b67de` (2026-07-11).** Все 80 последующих push'ей в `main` (включая Атлас родословия v1 `0aee617` и карту Авраама `2ca2af3b`) падают на шаге `Static publication gates` (56 failure + 24 cancelled, 0 success). Контент серий «Сердца» (А3–Д1), Нагорная-фиксы, hard-texts description-trim, Атлас v1 **не доезжают до продакшена.** Last-green: run `29138555390` @ `007b67de`. Требуется FAST-loop локально (`guard:shared-files`, `data:consistency`, `migration:metadata:check:strict`, `audit-pro`) для локализации конкретного гейта; не мержить новые фичи до зелёного деплоя. | `reverify/CURRENT_HEAD_REVERIFY_2026-07-14_2ca2af3b.md`; gh API 80 CI runs; `incoming/arena-auditor-2026-07-14/2026-07-14/evidence/ci-deploy-*.json` |
...
## Статистика (обновлено 2026-07-14 сессией arena-auditor: structural repair + reverify до `2ca2af3b`; +1 P0 DEPLOY-RED, +1 P2 ATLAS-D-NAMESPACE, +1 AR (AR-006); закрыто/реклассифицировано 0 в этой сессии; продакшен 🔴)
