...
| D-22 | Favorites/izbrannoe: `f.path`→href без проверки схемы (само-XSS) + protocol-relative `//host` в image — **fixed 2026-07-08** (`365de50`), НО **PARTIAL REGRESSION 2026-08-05** (#1061 переписал файлы): href-guard (`normalizePath`, same-origin) сохранён и XSS-safe; image-часть (`normalizeImage`, favorite-store.js:40-50) проверяет только `protocol`, НЕ `origin` → `//evil.com/x.png` и `https://evil.com/x.png` проходят в `background-image`. Не XSS (background-image), но exfiltration/дрейф от контракта. Рекомендация: добавить `url.origin === location.origin`. Evidence: `reverify/CURRENT_HEAD_REVERIFY_2026-08-05_d0647b71_favorite-store-d22.md` | `365de50` + #1061 |
...
| MAP-P1-11 | 🆕 **Karty P1:** Scale bar использует константу `cfg.W0` (1900) вместо актуальной `renderedWidth` (`canvas.getBoundingClientRect().width`, доступна map-engine.js:1108): `screenPxPerKm=(cfg.W0/view.w)*pxPerKm` (map-engine.js:1447) при canvas `width:100%` → mobile 390px ~4.9x, desktop 1200px ~1.6x ошибки масштаба (уточнено self-audit 2026-08-05) | verified-source (3a05a1e7) |
...
| STRANGLER-HYGIENE | 54 legacy index.html (11 корневых подпапок + 12 articles/ + глубже) при 83 Astro-страницах — дублирующийся legacy HTML; работает через page-ownership, но техдолг (подтверждено self-audit 3a05a1e7) (работает корректно через page-ownership, но техдолг). |
...
| NG-STRUCT-01 | 🆕 **Нагорная P1:** Сломанная структура заголовков секций — ch.2/SectionX, ch.5/SectionI–IV/X не имеют (подтверждено self-audit 3a05a1e7: обёртки `group mb-6 mt-12` есть в chast-1/4, отсутствуют в chast-2/3/5 SectionX) `<div class="group mb-6 mt-12">` обёртки (нет иконки, подзаголовка, отступа). Регресс Astro-миграции. + Emoji вместо SVG (19 секций ch.2/ch.5) + `font-sans` на h2 (4× ch.5). Evidence: `evidence/NAGORNAYA_VISUAL_AUDIT_2026-07-14.md` + `evidence/NAGORNAYA_DEEP_DARK_THEME_AUDIT_2026-07-14.md` |
...
| AR-IDX-CSS-02 | `.home-v20 { overflow-x:hidden }` клиппит Astro-фразы `h-ambient-native` (`width:100vw; translateX(-50%)`, HomePageChromeStyles:232); legacy `.h-scripture-bg` отсутствует в Astro (уточнено self-audit 2026-08-05). | `incoming/arena-auditor-index/2026-07-14/REPORT.md` §1; self-audit 3a05a1e7 |
...
### 2026-08-05 — Self-audit precision pass @ Product `3a05a1e7` (matrix wording)

Five rows refined from full re-verification (control audit 55+ checks, contracts 10):
1. **MAP-P1-11**: root cause refined — not 'canvasWidth' but 'constant `cfg.W0`=1900 vs actual `renderedWidth` (`canvas.getBoundingClientRect().width`, :1108)'. `map-engine.js:1447` `(cfg.W0/view.w)`. canvas `width:100%` → mobile 390px ≈4.9×, desktop 1200px ≈1.6× scale error.
2. **AR-IDX-CSS-02**: clipping object refined — Astro `h-ambient-native` (`width:100vw; translateX(-50%)`, HomePageChromeStyles:232) inside `.home-v20`; legacy `.h-scripture-bg` absent in Astro.
3. **SEARCH-P2-07**: corpus = 45 UNIQUE books (synodal 24 + kassian 21, zero overlap), 21/66 uncovered (was '300 records / 24 missing').
4. **NG-STRUCT-01**: `group mb-6 mt-12` wrappers present in chast-1/4, absent in chast-2/3/5 SectionX — matrix wording (ch.2/ch.5) confirmed.
5. **STRANGLER-HYGIENE**: 54 legacy index.html (11 root subdirs + 12 articles/ + deeper) vs 83 Astro pages — CONFIRMED OPEN, NOT closure candidate (my earlier claim was wrong; fixed).

Arithmetic unchanged: **371 = 226 closed + 145 open**. No row moved.
...
- Closed `SEARCH-P2-08` from Product PR #901, exact head `c99af2f104194d022e7f55092af6ad35e561de7b`, squash merge `b8882bf04a178d7a1d798a0377083ba57d29ce8a`.
