...
| Last reverify | `reverify/CURRENT_HEAD_REVERIFY_2026-08-01_b4b02f72_source-vs-production.md` |

⚠️ Deploy-формулировки в исторических строках ниже сохраняют состояние соответствующей даты. Current source = `b4b02f72c26f5ac9c58ea9efe11cfcf4fa3d2c19`; last exact production authority = `abf1edba190280e554dfda085bef9fb6594c896d`. Более новые source-коммиты не наследуют live/production-доказательства старого SHA. Draft PR #667 остаётся active/unmerged и не входит в current source truth. Evidence: `reverify/CURRENT_HEAD_REVERIFY_2026-08-01_b4b02f72_source-vs-production.md`.
...
| NEW-VOSK-FETCH-NO-ABORT | 🆕 reverify 07-14 (verified-source `2ca2af3b`): 280MB model `fetch(MODEL_URL)` not wired to AbortController; continues after navigation/opt-out. `js/vosk-tts-engine.js:166`. Reported by claude-auditor 07-09, confirmed 07-14. |
| AR-AUDIT-17 | 🆕 reverify 07-14 (verified-source `2ca2af3b`): `validate:all` fails with 2 inline script syntax errors in `scripts/genealogy-build/atlas-template.html` and `interactive-template.html`. Genealogy build templates — not production code, but gate regression. |
...
## Статистика (обновлено 2026-08-01: source `b4b02f72`; last exact production `abf1edba`; counters unchanged)
...
- **2026-08-01 — source advanced to `b4b02f72`; production remains `abf1edba`.** Current source includes bug-hunt repairs through `be970bfc`, Atlas geometry PR #659 / `65bf6c4a`, Avraam heading lifecycle PR #665 / `8a8ebf70`, Karty story-ID schema PR #666 / `424b09b2` and README Astro 7 truth PR #668 / `b4b02f72`. AuditRepo PR #112 / `2ef6cf66` records exact Karty evidence. Last exact production remains deploy `30669840189` attempt `1` at `abf1edba`; no same-SHA production witness exists for current source. Draft source PR #667 is active/unmerged and excluded. This authority-only synchronization changes no bug rows or counters: 164 closed / 192 open. Evidence: `reverify/CURRENT_HEAD_REVERIFY_2026-08-01_b4b02f72_source-vs-production.md`.
