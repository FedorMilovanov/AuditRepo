...
| `GBS-SEARCH-CONTROL-PLANE-001` | Same-day reverify keeps this external root current. The readable URL-prefix property `https://gospod-bog.ru/` still has **0 submitted Search Console sitemaps** even though live `robots.txt` advertises two reachable production sitemaps. The tracker now reports **92 total / 36 indexed / 56 not indexed / 0 errors / 0 warnings**; primary sitemap performance finds 21/92 URLs with GSC impressions. `sc-domain:gospod-bog.ru` still returns 403. Bing is connected but has 0 traffic/crawl/feed rows while submission quota remains available, so the state is not explained by an API-key/quota failure. GSC Wizard metadata now mirrors the production sitemap URLs for reporting only; that did not change the authoritative empty Search Console sitemap list. Product tracking issue: `gb-is-my-strength#1996`. | Submit the production sitemap(s) in the working URL-prefix property and verify Google fetch/processing. Separately either verify `sc-domain:gospod-bog.ru` with the exact Google DNS TXT at the Cloudflare apex or explicitly retire/ignore that inaccessible property. Then re-run URL Inspection after recrawl before promoting page-specific content defects. Do not create a second trial-only IndexNow authority merely to make an aggregator dashboard green. Detailed receipt: `../reverify/CURRENT_HEAD_REVERIFY_2026-09-12_search-control-plane.md`. |
...
- `SECURITY-CSP-INCONSISTENCY` + `SECURITY-CSP-GAPS` → manifestations of active `FRAGMENTED-SECURITY-OWNERSHIP`.
