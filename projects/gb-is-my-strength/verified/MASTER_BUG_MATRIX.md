...
| UI-GILL-SUBMENU-ANCHOR-02 | P1 functional/navigation | **fixed-current** | `gillSeriesData.ts` Part III had two rows both pointing to `#sec-church-gov` (distinct concepts: "Гилл и Рим" vs "Coffee House и права поместной церкви"). Split into `#sec-church-gov` + new distinct `#sec-church-gov-polity` (added as a real sub-anchor on the church-membership paragraph in `GillPart3ArticleBody.astro`). `data/gill-submenu-anchor-reconciliation.json` records the full 7-rename map (the Part III dup is one of them; the other 6 are legacy migration renames for which labels/order are identical). The audit now also REJECTS duplicate hrefs/labels (was previously approving the bug because both source and the hardcoded EXPECTED shared it). |
| UI-GILL-HISTORICAL-PARITY-03 | P1 | **reference-manifest-committed + reconciled** (browser run pending remote CI) | `scripts/gill-pre-v16-submenu-regression-audit.js` hardened: full per-item scroll traversal (exact active row, passed count = i, counter `i+1 / TOTAL`, active-row visibility). The historical reference manifest `data/gill-pre-v16-submenu-reference.json` is now **generated from `bcf6389f…` and committed**; it is reconciled against **7 Astro-migration anchor renames** via `data/gill-submenu-anchor-reconciliation.json`. The audit normalizes the manifest to array form AND applies the reconciliation map, so `EXPECTED` == current source with **0 duplicate hrefs** (verified statically). Remaining: green remote CI browser run + owner visual parity review. |
