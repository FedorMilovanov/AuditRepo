...
| Поле | Значение |
|---|---|
| Source HEAD | `bd8cb9a0` (main; актуально `git ls-remote origin main`, коммит 2026-07-13 20:17Z). ⚠️ Прежний `b8459bdf` **не существует как git-объект** — история разошлась (P2-SSOT-DRIFT-2026-07-14). |
| Deploy | 🔴 **RED** (на `bd8cb9a0`, verified 2026-07-14): Deploy #1568 fail (`css:layer:validate` 210>202), Metadata&IndexNow #1330 fail (5 metadata records missing), Visual Parity #383 fail; Native Source Contract #151 ✅. Прежняя запись «GREEN @ `29065454930`@b8459bdf» ссылалась на несуществующий SHA. |
...
> **🔴 Release-contract сломан (verified 2026-07-14, HEAD `bd8cb9a0`).** Три deploy-blocking находки
> из контент-сессий 2026-07-11..13 (Нагорная проповедь + серия «Сердце» + Gill Ч.4). Полное evidence:
> `reverify/CURRENT_HEAD_REVERIFY_2026-07-14_bd8cb9a0_css-important-gate-drift.md`; синтез:
> `working/VERIFIER_SYNTHESIS_2026-07-14_css-important-gate-drift.md`; интейк:
> `incoming/gpt-css-important-audit/2026-07-14/`. **Все три латентны/последовательны** — деплой
> падает на первом (CSS), два других всплывут после его починки; зелёный требует всех трёх.
> Правка — release-транзакция (SUPER_AUDIT W1) + freeze-зоны (Нагорная-визуал) → координировать с владельцем.

| ID | Описание | Witnesses |
|---|---|---|
| P1-CSS-IMPORTANT-GATE-DRIFT | 🆕 `css/site.css` = **210** `!important` → нарушает ОБА гейта: `css:layer:validate` ceiling **202** (`package.json:123`) и `audit-pro.js` ceiling **200** (`:85`). **Реальный деплой падает** именно здесь: Deploy #1568 (run `29281815427`) → `css:layer:validate` «!important count 210 exceeds ceiling 202» → exit 1. Root cause (verifier): регрессия 200→210 в 2 коммитах 2026-07-13 `26266ee`(+8)/`e904670`(+2) «Нагорная проповедь» — **это настоящие WCAG-фиксы контраста/reduced-motion**, не долг → развязывать архитектурно (scoped selector/`@layer`/выше специфичность), НЕ удалять. Понизить до 202 недостаточно (audit-pro требует ≤200). Ceiling только вниз (ratchet). | GPT-интейк + verifier: source+build+**CI-лог**, `verified-source`/`verified-build` |
| P1-EDITORIAL-METADATA-REGISTRY-GAP | 🆕 (verifier-new, независим от CSS) Workflow «Metadata & IndexNow Readiness» #1330 (run `29281815491`) 🔴: `Eligible 25 / Registry 20` → **5 записей отсутствуют** в `data/editorial-metadata.json`: `dzhon-gill-chast-4-ekzeget`, `chto-bibliya-nazyvaet-serdcem`, `novoe-serdce`, `serdce-i-duh`, `serdce-spravochnik` (все — новые статьи 07-11..13, editorial-freeze baseline не досинхронизирован). Проверено: 5/5 slug'ов реально отсутствуют в файле. Подтверждает оговорку GPT «CSS ≠ единственная причина». Fix: до-сгенерировать записи. | verifier: source+**CI-лог**, `verified-source` |
| P1-NAGORNAYA-JS-UNREGISTERED | 🆕 (verifier-new, latent deploy-blocker #3) `js/nagornaya-bar-extras.js` (добавлен `1c41b15`, подключён во всех 5 `NagornayaChastN PageFooter.astro` как `?v=1`) **отсутствует в** `ALLOWED_JS` (`audit-pro.js:52-67`) → hard-fail «Forbidden JS files in js/» **и** в `cache-bust-assets.js` → нет реального cache-bust (застрял на `?v=1`). Живой инстанс `GATE-MARKER-DATA-DRIFT` (см. P3). Латентен за CSS-гейтом. Fix: внести файл в оба списка + реальный cache-bust. | verifier: source+build (`audit-pro` exit 1), `verified-source` |
...
| ID | Описание | Witnesses |
|---|---|---|
| P2-CSS-SYNTAX-SITECSS | 🆕 (GPT §7 + verifier, все подтверждены прямым pattern-search на `bd8cb9a0`) 4 грамматических ошибки + 1 повреждённый селектор в `site.css`, ПРОХОДЯТ действующие валидаторы (brace-only): **SYNTAX-001** пустое reduced-motion-правило `@media(prefers-reduced-motion:reduce){.bottom-bar,.btoc-link,.flip-card-inner,.h-article-card,.quiz-option}` (a11y-защита не применяется); **SYNTAX-002** висячий `.ehrman-block,.info-box,.quote-box}` (контраст не применяется); **SYNTAX-003** `…,.resume-reading-title,@supports(animation-timeline:scroll())` — at-rule как член selector list; **SYNTAX-005** `.gbx-backlinks__maplink:rgba(122,46,46,0.08);gbx-backlinks__maplink:hover{…}` (`:rgba()` как псевдокласс, `;` в prelude, нет `.`); (+ **DEAD-004** пустой `html.dark` — P3). Восстанавливать по git history/blame (первые появления `a426e1a`, серия «Сердце»), НЕ по догадке. Делает D-2 не теоретическим (реальные witnesses). | GPT-интейк + verifier `verified-source` |
| P2-SSOT-DRIFT-2026-07-14 | 🆕 (GPT §1 + verifier, усилено) Матрица трекала `b8459bdf`, который **не существует как git-объект** (`git cat-file -t b8459bdf` → «Not a valid object name»); шапка «Deploy ✅ GREEN @ run 29065454930@b8459bdf» ссылалась на несуществующий SHA, тогда как current `main` `bd8cb9a0` **RED**. Мастхед/`NEXT_AGENT_PROMPT` синхронизированы на `bd8cb9a0` этой сессией; строки-witnesses (номера, счётчики) перепривязаны на current HEAD (см. §historical rechecks). | verifier `verified-source`+CI |
...
| CI-INDEXNOW-CHECKER-STALE | 🆕 2026-07-09: workflow `native-source-contract` **красный на PR-ах, затрагивающих `src/pages|components|content`** (триггер только `pull_request`; на push в `main` НЕ запускается → ветку main НЕ блокирует: на HEAD `7a410be9` main зелёный, deploy+guard success). Pre-existing, ещё на базе `b60296b1`. `check-workflows.js:157/198` требует у `indexnow.yml` `contents: write` («for metadata commit») и `baptisty-rossii/**` в paths. Но `indexnow.yml` теперь **read-only readiness-гейт**: 0 commit/push-шагов, `cache-bust.js` без `--write`, шаг «ensure tree clean» — писать нечего, `contents: read` корректен (least-privilege). Правило чекера — реликт до-NEW-53 архитектуры (indexnow-сабмишн+cache-bust-commit переехали в `deploy.yml`); комментарий-шапка `deploy.yml` («после cache-bust-коммита») тоже устарел. **Не чинил автономно ночью** — область release-транзакции (SUPER_AUDIT W1), правка прав/гейтов пайплайна требует решения владельца. Кандидат-фикс: обновить `check-workflows.js` — снять требование `contents: write` с read-only indexnow, перенести indexnow-submission/baptisty-coverage требования на `deploy.yml` (где реально происходят); плюс освежить stale-комментарий deploy.yml. | reverify 2026-07-09, verified-source |
...
| D-2 | css-layer-validator: заголовок обещает проверку порядка @layer, код проверяет только необъявленные слои; порог <50% против цели ≥80%; валидирует только site.css. **🔧 Расширено 2026-07-14 (verifier):** оба валидатора считают только баланс скобок, НЕ грамматику (лог: «Brace balance: OK» при 5 реальных дефектах — см. P2-CSS-SYNTAX-SITECSS); duplicate-selector heuristic **пропускается для файлов ≥250000 chars** (site.css=291092 → пропущен); layered = **21.2%** (было 21.9%). Фикс: настоящий CSS-parser + валидировать все core-stylesheets + не пропускать большие файлы + selector-grammar. | arena cycle2 + verifier 07-14 `verified-source` |
...
| ID | Описание |
|---|---|
| P3-CSS-DUP-CLEANUP | 🆕 (GPT §9 + verifier, все счётчики подтверждены на `bd8cb9a0`) Механически-безопасный CSS-cleanup: дубль `html{scroll-behavior:smooth}` ×2 в `site.css` (Δ!important 0); дубль `html.dark body.nagornaya-page .summary-card{…!important}` ×2 в `nagornaya-mobile-toc.css` → **единственное подтверждённое безопасное снижение `!important`: 135→134** (после — опустить ceiling 135→134); nested `var(--color-text,var(--color-text,#1a1a1a))` ×3 + `var(--color-text-muted,var(--color-text-muted,#78716c))` ×2 (упростить); пустой `@media(hover:hover)and(pointer:fine){html.dark }` (проверить blame перед удалением). Не решает `site.css 210>200`. |
...
| BUG-011 | 23 unique breakpoints, 768px collision. 🔧 **Recheck 2026-07-14:** теперь **57** уникальных `px`-брейкпоинтов в `css/*.css` (вырос с 23); near-collision кластер 760/761/768 присутствует. `confirmed-current (worse)` |
...
| D-3 | JS total 375041 > 365000 (бюджет audit-pro); CSS-бюджет в норме. 🔧 **Recheck 2026-07-14:** JS total (14 allowed + sw.js) = **469101** > 365000 (вырос с 375041, гл. обр. Vosk-движки); это `R.warn`, **не** блокирует деплой. `confirmed-current (worse)` |
| D-4 | Magic z-index: токены `--z-*` существуют, фикс тривиален (⚠️ PremiumControls in-flight — согласовать). 🔧 **Recheck 2026-07-14:** audit-pro флагует **8** инстансов — `mobile-hotfix.css` ×1 (`2102`), `floating-cluster.css` ×7 (`2102/9999/3000/2147483000×3/2147483100`); прежние номера строк (`2372/2447/2504/2697/2882`,`129`) устарели — нужна re-attribution. `confirmed-current` |
...
## Статистика (обновлено 2026-07-14 сессией CSS-`!important`-gate-drift: +3 P1 [CSS-gate, metadata-registry, nagornaya-js], +2 P2 [CSS-syntax, SSOT-drift], +1 P3 [CSS-dup-cleanup]; HEAD перепривязан `b8459bdf`→`bd8cb9a0`; deploy GREEN→RED)
...
| Рефакторинг | 4 |
| AuditRepo | 3 |
| **Всего открыто (матрица)** | **50** |
| Системный бэклог вне матрицы | см. `SUPER_AUDIT_2026-07-06_14a49be8.md` (волны W1–W10) |
| False positives отклонено | 5 |
| Passes processed | 96+ (reverify 2026-07-14 @ `bd8cb9a0`, claude-auditor; интейк gpt-css-important-audit) |
...
- **2026-07-14 — Верификация внешнего CSS-`!important`-аудита (GPT) + перепривязка HEAD; прод оказался КРАСНЫМ.**
  Интейк `incoming/gpt-css-important-audit/2026-07-14/` (внешний GPT, report-only). Верификатор (claude-auditor)
  сверил на authoritative `main` `bd8cb9a0` (`git ls-remote`) и подтвердил ядро отчёта **реальным падающим CI**.
  - **Матрица трекала `b8459bdf` — его нет как git-объекта** (`git cat-file` → «not a valid object»); шапка «Deploy GREEN @ run 29065454930» ссылалась на несуществующий SHA. Прод фактически 🔴 RED (3 workflow: Deploy #1568, Metadata&IndexNow #1330, Visual Parity #383). Мастхед + `NEXT_AGENT_PROMPT` синхронизированы на `bd8cb9a0`. → `P2-SSOT-DRIFT-2026-07-14`.
  - **+3 P1 deploy-blocker (все из контент-сессий 07-11..13, координировать с владельцем — release-транзакция W1 + Нагорная-freeze):** `P1-CSS-IMPORTANT-GATE-DRIFT` (`site.css` 210>202/200; deploy падает на `css:layer:validate`, CI-лог run `29281815427`; root-cause коммиты `26266ee`/`e904670` — **реальные WCAG-фиксы**, развязывать архитектурно, не удалять); `P1-EDITORIAL-METADATA-REGISTRY-GAP` (5 записей нет в `editorial-metadata.json` → Metadata workflow, независим от CSS); `P1-NAGORNAYA-JS-UNREGISTERED` (`nagornaya-bar-extras.js` не в `ALLOWED_JS`+`cache-bust-assets` → audit-pro «Forbidden JS», latent). Sequencing: 3 барьера последовательны — зелёный требует всех трёх.
  - **+2 P2:** `P2-CSS-SYNTAX-SITECSS` (4 грам. ошибки + повреждённый backlinks-селектор, проходят brace-only валидатор); D-2 расширен реальными witnesses (brace-only, dup-selector skip ≥250k, 21.2%). **+1 P3:** `P3-CSS-DUP-CLEANUP` (2 дубля + nested `var()`; единственный безопасный ratchet-down `!important` = nagornaya 135→134).
  - **Verifier-additions поверх GPT:** точный счётчик `floating-cluster.css` = **503** (GPT оставил неподтверждённым; 503≤524 ratchet OK, но 5× goal 100); 4-й красный workflow (Visual Parity); root-cause коммиты; латентные гейты #2/#3. **Речек carry-over:** BUG-011 23→**57** брейкпоинтов; D-3 375041→**469101** JS (warn); D-4 → 8 z-index, строки устарели; D-2 21.9%→21.2%.
  - **Ничего не закрыто** (source не менялся — аудит-only); L4/repair-ready не выдан (freeze-зоны + a11y-природа +10 директив). Evidence: `reverify/CURRENT_HEAD_REVERIFY_2026-07-14_bd8cb9a0_css-important-gate-drift.md`; синтез: `working/VERIFIER_SYNTHESIS_2026-07-14_css-important-gate-drift.md`.
