...
| NG-DARK-04 | 🆕 **Нагорная P2:** `bg-rose-50` без dark-ремапа — 13 контейнеров в ch.5 остаются #fff1f2 (светло-розовый) в тёмной теме. **Подтверждено цикл 4:** `bg-rose-50` ОТСУТСТВУЕТ в blanket-группе `.bg-*-50` в `mobile-hotfix.css` (перечислены amber/blue/emerald/green/neutral/orange/purple/red/slate/stone/teal/white/yellow/zinc — НО НЕ rose). Решается через per-chapter `var(--ng-accent-soft)`. Evidence: `evidence/NAGORNAYA_DEEP_DARK_THEME_AUDIT_2026-07-14.md` + `evidence/NAGORNAYA_DEEP_AUDIT_CYCLE4_2026-07-14.md` §5.2 |
...
| NG-A11Y-01 | 🆕 **Нагорная P3:** Emoji вместо SVG иконок (18 секций: 10 ch.2 + 8 ch.5) — рендеринг зависит от ОС, не масштабируется; ch.2/VIII использует «#» вместо emoji; inline hero height `style="height:320px"` не адаптивен. Evidence: `evidence/NAGORNAYA_DEEP_AUDIT_CYCLE3_2026-07-14.md` |
| NG-CROSS-01 | 🆕 **Нагорная P3:** Кросс-главные цветовые утечки — 20+ экземпляров не-акцентных цветов: ch.2  (Ipsissima vox box), ch.4  ×8 (Concursus таблица), ch.5 //. Не ломает визуал сейчас, но затрудняет миграцию на CSS vars. Evidence: `evidence/NAGORNAYA_DEEP_AUDIT_CYCLE4_2026-07-14.md` §2.2 |
| NG-SERIYA-01 | 🆕 **Нагорная P3:** Seriya page без `bg-stone-100` на `<body>` — единственная из 9 nagornaya-страниц без него (есть `nagornaya-series-page` вместо). Визуальная несогласованность + нужен `data-chapter` для CSS vars. Evidence: `evidence/NAGORNAYA_DEEP_AUDIT_CYCLE4_2026-07-14.md` §8 |
...
## Статистика (обновлено 2026-07-14 cycle 4: нагорная архитектурный профиль + 3 новых бага NG-CROSS-01/NG-SERIYA-01/NG-TOC-01; уточнённые счётчики accent/border/bg)
...
- **2026-07-14 — Нагорная проповедь глубокий аудит Цикл 4 (arena-auditor).**
  Углублённый количественный аудит всех 115 Astro-компонентов. Source HEAD: `21624a3e`.
  **Уточнённые счётчики:** 374× `text-{accent}-600` без dark remap (было 165), 108× `text-{accent}-700` (было 75), 168× `border-stone-100` (было 52), 152 inline `style=` (было 172).
  **Новые баги:** NG-CROSS-01 (кросс-главные цветовые утечки, 20+ экз.), NG-SERIYA-01 (seriya page без bg-stone-100), NG-TOC-01 (TOC accent не per-chapter).
  **Подтверждено:** NG-DARK-04 (`bg-rose-50` отсутствует в blanket dark remap в mobile-hotfix.css), NG-BODY-01 (bg-stone-100 на body не ремапится).
  **Professional solution (уточнённый):** `css/nagornaya-chapter-vars.css` с `data-chapter="N"` + `--ng-accent`/`--ng-accent-text`/`--ng-accent-soft`/`--ng-border-soft` custom properties.
  Evidence: `incoming/arena-auditor/2026-07-14/evidence/NAGORNAYA_DEEP_AUDIT_CYCLE4_2026-07-14.md`.
