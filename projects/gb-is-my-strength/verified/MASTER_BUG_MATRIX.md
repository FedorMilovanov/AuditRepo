...
> Arithmetic: 0 direct current defects + 0 improvements + 0 residuals + 1 system verification lane + 0 owner decisions = 1 independent repair owner. Named manifestations such as CSP variants/gaps and nosniff meta misuse remain closure witnesses under their causal owner; they are not separate active rows.
...
- `EDITORIAL-LABEL-INCONSISTENCY` → manifestation of the now-closed `METADATA-SSOT-PROLIFERATION` owner.
- `RSS-SERIES-DATE-COLLAPSE` → public-artifact witness under the now-closed `METADATA-SSOT-PROLIFERATION` owner.
