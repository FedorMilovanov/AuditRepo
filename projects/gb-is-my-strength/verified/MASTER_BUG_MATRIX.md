...
| Deploy | 🔴 **RED / STALE** — last GREEN `007b67def5` @ 2026-07-11T03:46Z (run `29138555390`). HEAD deploy `29338523013` FAIL on **Static publication gates**. Since 07-11: ~59 fail + ~25 cancel / ~85 runs. Readers ≠ main. |
...
| Last reverify | `reverify/CURRENT_HEAD_REVERIFY_2026-07-14_2ca2af3b.md` |
...
## 🟠 P1 — ОТКРЫТО (6)
=======
## 🔴 P0 — ОТКРЫТО (5) — release / deploy unblock

| ID | Описание | Witnesses |
| S-DATA-01 | Series-Route Desynchronization (JSON vs Filesystem) | Auditor 2026-07-14 |
| S-T-01 | Audit-Pro Guard Blind Spot: Astro/MDX routes invisible to guard | Auditor 2026-07-14 |
...
| DEP-BLOCK-EDITORIAL-REGISTRY | 🆕 `editorial-metadata-registry.js --check` exit 1: eligible 25 / records 20. Missing: `dzhon-gill-chast-4-ekzeget`, `chto-bibliya-nazyvaet-serdcem`, `novoe-serdce`, `serdce-i-duh`, `serdce-spravochnik`. Registry `sourceCommit` still `e6793627`. **Ломает IndexNow readiness** (`Validate registry structure`). | verified-source reverify 07-14 |
| DEP-BLOCK-MAPS-VALIDATE | 🆕 `maps:validate` exit 1, 25 issues: (1) hub «на аудите»=9 но missing cards=10 после `nachalo`; (2) `nachalo/route.json` schema incomplete; (3) avraam meta.stats places 19≠22 + stage-less babylon/mari/paran; shoftim stats. В цепочке `validate:static-publication`. | verified-source |
| DEP-BLOCK-CSS-IMPORTANT-CEILING | 🆕 `css:layer:validate` exit 1: site.css `!important` **210 > ceiling 202**. Instance of D-2 now **deploy-blocking**. Layered still ~21%. | verified-source |
| DEP-BLOCK-AVRAAM-AUDIT | 🆕 `avraam:audit` 25/27 fail: route places 22 vs HTML set 19 (extra babylon/mari/paran-region). In static publication chain. | verified-source |

## 🟠 P1 — ОТКРЫТО (5)
>>>>>>> fc5f7b6e65624eb5330b0bd098aead9311cfb3ca
...
| NG-VIS-04 | 🆕 **Нагорная P2:** Табличная перегрузка — 8 секций без текстовых абзацев (ch.2/III/V/IX/X, ch.3/V/VII/VIII, ch.5/III). Только гриды/карточки/таблицы — нет «воздуха». ch.2 имеет 1.5x structured/text ratio. Evidence: `evidence/NAGORNAYA_VISUAL_AUDIT_2026-07-14.md` |
| NG-VIS-05 | 🆕 **Нагорная P2:** Мёртвый класс `reveal` — используется в ch.4 (1×) и ch.5 (10×), но нет `.reveal` CSS для nagornaya-страниц (только `body.gbs-world .reveal`). Either dead code или missing scroll-reveal. Evidence: `evidence/NAGORNAYA_VISUAL_AUDIT_2026-07-14.md` |
| NG-VIS-06 | 🆕 **Нагорная P2:** `font-sans` на h2 заголовках только в ч.5 — нарушение единства серии (остальные 4 главы используют serif). Evidence: `evidence/NAGORNAYA_VISUAL_AUDIT_2026-07-14.md` |
| NG-VIS-07 | 🆕 **Нагорная P2:** Потеря цветовой идентичности глав в тёмной теме — mobile-hotfix.css blanket-override маппит все акцентные цвета (blue/emerald/purple/rose-800) → одинаковый `rgba(238,231,220,.78)`. В светлой теме каждая глава = уникальный цвет; в тёмной = все одинаковые. Evidence: `evidence/NAGORNAYA_VISUAL_AUDIT_2026-07-14.md` |
| NG-VIS-08 | 🆕 **Нагорная P2:** Возможный контраст-дрейф `text-stone-400` внутри `bg-stone-900` hero-блока (ch.3/VIII) в тёмной теме из-за blanket color remap. Evidence: `evidence/NAGORNAYA_VISUAL_AUDIT_2026-07-14.md` |
...
| NG-VIS-01 | 🆕 **Нагорная P1:** Невидимый текст «Из библиотеки» в тёмной теме — inline `color:#1c1410` / `color:#8a7968` / `background:#faf8f5` на всех 5 частях (SectionX). Override `mobile-hotfix.css` → `.gb-series-link` не пробивает inline `style=`. Evidence: `evidence/NAGORNAYA_VISUAL_AUDIT_2026-07-14.md` |
| NG-VIS-02 | 🆕 **Нагорная P1:** Сломанная структура заголовков секций — ch.2/SectionX, ch.5/SectionI–IV/X не имеют `<div class="group mb-6 mt-12">` обёртки (нет иконки, подзаголовка, отступа). Регресс Astro-миграции. Evidence: `evidence/NAGORNAYA_VISUAL_AUDIT_2026-07-14.md` |
| NG-VIS-03 | 🆕 **Нагорная P1:** Emoji вместо SVG в иконках секций — ch.2 (10 секций: ❓👨‍🏫📄📋💬📣#🔭🏛️🧩), ch.5 (9 секций: 🕊️🔑⚖️🚪🚨💛🛡️💡✨). Платформозависимый рендеринг, нарушение дизайн-системы. Evidence: `evidence/NAGORNAYA_VISUAL_AUDIT_2026-07-14.md` |
...
## Статистика (обновлено 2026-07-14 HEAD reverify @ `2ca2af3b` + arena-auditor governance pass + нагорная visual audit: +3 P1 [NG-VIS-01/02/03], +5 P2 [NG-VIS-04–08], +4 P3 [NG-VIS-09–12], D-1 P2→P3)
...
- **2026-07-14 — Нагорная проповедь visual audit (arena-auditor).**  
  ...
  **+3 P1:** NG-VIS-01 (invisible dark-theme text in «Из библиотеки»), NG-VIS-02 (broken section headers ch.2/ch.5), NG-VIS-03 (emoji instead of SVG in ch.2/ch.5).  
  ...
  **New matrix IDs:** PROD-STALE-DEPLOY-RED, DEP-BLOCK-EDITORIAL-REGISTRY, DEP-BLOCK-MAPS-VALIDATE, DEP-BLOCK-CSS-IMPORTANT-CEILING, DEP-BLOCK-AVRAAM-AUDIT, HUB-AUDIT-COUNT-DRIFT, GENEALOGY-ATLAS-V1-SHIPPED-NOT-PROD.  
  **Confirmed still open:** D-1 concurrency cancel-in-progress both workflows; D-2 (now blocking); D-21; SW still gb-v189; BaseLayout `Date.now()` version.  
  **Product delta not on prod:** genealogy atlas v1 (owner milestone), heart-series MDX wave, mobile-chrome, atlas sheets/nachalo/avraam enrichment.  
  **Unblock lane:** registry write (5 routes) → hub 9→10 → nachalo schema/exclude → avraam stats → css ceiling → green deploy.  
  **SSOT:** masthead + NEXT_AGENT updated this session (closes 4-day HEAD drift).
