...
| Поле | Значение |
|---|---|
| Source HEAD | `2ca2af3b` (main; merge: Библейский атлас — карта Авраама; генеалогия v1; сердечные статьи А–Д; фиксы Нагорной/karty/map/hard-texts) |
| Deploy | 🔴 **RED** — `deploy.yml` падает на **«Static publication gates»** с 2026-07-11 (80 runs = 56 failure + 24 cancelled, 0 success после `29138555390`@`007b67de`). Последний failure: `29338523013`@`2ca2af3b`. ⚠️ **RCA локализован (FAST-loop 2026-07-14):** после `cache-bust --write` `audit-pro` всё ещё падает на **11 ошибках** по трём группам: (1) гейты `audit-pro.js`/`validate.js` ходят `walk(ROOT)` **без skip `scripts/`** — build-шаблоны `scripts/genealogy-build/{atlas,interactive}-template.html` (placeholder `/*__ATLAS__*/;` — не валидный JS, без `lang`/canonical/charset/JSON-LD/theme-color) детектируются как прод-страницы → inline-script syntax + HTML-contract ошибки; (2) `js/nagornaya-bar-extras.js` не внесён в `ALLOWED_JS` в `audit-pro.js` (G04); (3) реальные регрессии — `css/site.css` !important 210 > potolok 200, репозиторные пути (`AuditRepo/...`) в docs/ATLAS-CONTRACT и genealogy-build/README, оверсайз `images/atlas-export/avraam-{hires,preview}.png`, 38 orphan-изображений (1605 КБ, в т.ч. og-* на 2 новые статьи «сердца»). Параллельно падает `editorial-metadata-registry` (5 новых статей-«сердец» не внесены в `data/editorial-metadata.json`) и `visual-parity` pixel-diff (новые страницы — нужен refresh baseline). |
| Системный бэклог | `SUPER_AUDIT_2026-07-06_14a49be8.md` — волны W1–W10, **вне счётчиков матрицы**; атлас-трек KA-0…KA-8 в `working/atlas/DEBT-REGISTER.md` и `incoming/claude-atlas-deep-audit/2026-07-10/` |
| Консолидация | 2026-07-05 (из монолита → `archive/2026-07-04-stale-matrix/MASTER_BUG_MATRIX_FULL_2026-07-03.md`); structural repair 2026-07-14 (root clutter + intake contract) |
...
|---|---|---|
| DEPLOY-STATIC-GATES-RED-2026-07-11 | 🔴 **Production застрял на `007b67de` (2026-07-11).** Все push'и в `main` (Атлас родословия v1 `0aee617`, карта Авраама `2ca2af3b`, 9 статей «Сердца», фиксы Нагорной/karty/map/hard-texts) падают на **трёх** параллельных workflow: (а) `deploy.yml` job `deploy` step **`Static publication gates`**; (б) `indexnow.yml` (aka Metadata & IndexNow Readiness) step **`Validate registry structure`**; (в) `visual-parity.yml` job `pixel-diff` step **`Run pixel-diff screenshots`**. Last-green: run `29138555390` @ `007b67de`. **RCA локализован (FAST-loop 2026-07-14, Node v22.22.3)**: после `npm ci && cache-bust --write` `audit-pro` всё ещё даёт **11 errors** (плюс 8 warnings), в т.ч. tooling-ложные срабатывания на build-шаблонах `scripts/genealogy-build/*.html` и `ALLOWED_JS` без нового `js/nagornaya-bar-extras.js`, плюс реальные: `css/site.css` !important 210 > ceiling 200, 2 `AuditRepo/...` path-leak в markdown, oversized `images/atlas-export/avraam-{hires,preview}.png` (16+1.7 MB), 38 orphan images; плюс `data/editorial-metadata.json` не содержит 5 новых статей `chto-bibliya-nazyvaet-serdcem/novoe-serdce/serdce-i-duh/serdce-spravochnik` + Gill-chast-4; visual-parity pixel-diff требует refresh-бейзлайна под новый визуал Атласа/Нагорной/сердец. Repair-lane: сначала фикс tooling-gaps (skip `scripts/` в audit-pro/validate walk'ах, добавить nagornaya-bar-extras в ALLOWED_JS), потом одной транзакцией cache-bust --write + regenerate editorial-metadata --write + почистить path-leak/oversized/orphans/!important + refresh pixel-diff baseline владельцем. Не мержить новые фичи до зелёного деплоя. | `reverify/CURRENT_HEAD_REVERIFY_2026-07-14_2ca2af3b.md`; FAST-loop evidence: `incoming/arena-auditor-2026-07-14/2026-07-14/evidence/audit-pro-{before,after-cachebust}.*`, `metadata-registry-before.txt`, `validate-strict-before.txt`; gh API 80+ CI runs; `ci-deploy-*.json` |
...
| AR-006 | validator root-dir gap (root allow-list для директорий и не-.md файлов, чтобы не допускать мусор вроде корневого `verification/atlas/` с 27 PNG) | `incoming/arena-auditor-2026-07-14/2026-07-14/REPORT.md` §1 (AUDITREPO-VALIDATOR-ROOT-DIR-GAP) |
...
## Статистика (обновлено 2026-07-14 второй проход arena-auditor: FAST-loop RCA на `2ca2af3b` — P0 DEPLOY-RED локализован (audit-pro scripts/ walk-gap + ALLOWED_JS gap + 6 реальных ошибок); metadata-registry и visual-parity baseline отдельно; +1 P2 ATLAS-D-NAMESPACE, +1 AR-006; продакшен 🔴)
...
- **2026-07-14 (второй проход, FAST-loop RCA) — arena-auditor-2026-07-14:** клон source @ `2ca2af3b` в `/tmp/gb-is-my-strength`, `npm ci` (Node v22.22.3, 476 пакетов, 3.5 GB RAM свободно, OOM не случился). Прогнан FAST-цикл: `guard:shared-files ✅`, `data:consistency ✅`, `migration:metadata:check:strict ✅`, `audit-pro ❌` (114 errors до cache-bust, **11 errors после cache-bust --write**), `validate:strict ❌`, `editorial-metadata-registry ❌ (5 missing records)`. **P0 DEPLOY-STATIC-GATES-RED-2026-07-11 локализован по root-cause:** (1) tooling-gap: `audit-pro.walk(ROOT)` не скипит `scripts/` → build-шаблоны `scripts/genealogy-build/{atlas,interactive}-template.html` с placeholder `/*__ATLAS__*/;` и без `lang`/canonical/charset ловятся как прод-страницы → 6 ошибок inline-script syntax/html-contracts/warnings JSON-LD/sitemap/theme-color; то же в `validate.js:validateInlineScripts`; (2) ALLOWED_JS в `audit-pro.js` не содержит `js/nagornaya-bar-extras.js` (подключается Astro-футерами chast-1..5); (3) реальные регрессии — `css/site.css` !important 210 > ceiling 200, 2 repository-path leak в markdown (`docs/ATLAS-CONTRACT-2026-07-10.md`, `scripts/genealogy-build/README.md`), oversized `images/atlas-export/avraam-hires.png` (16 MB) и `avraam-preview.png` (1.7 MB), 38 orphan images (~1.6 MB, в т.ч. og-* для 2 новых «сердечных» статей). Параллельно падают: `Metadata & IndexNow Readiness` (registry structure) — `data/editorial-metadata.json` не имеет записей для 5 новых маршрутов (`dzhon-gill-chast-4-ekzeget`, `chto-bibliya-nazyvaet-serdcem`, `novoe-serdce`, `serdce-i-duh`, `serdce-spravochnik`); `Visual Parity Guard — pixel-diff` — новые страницы Атлас/Сердца/Нагорная требуют owner-approved baseline refresh. Evidence: `incoming/arena-auditor-2026-07-14/2026-07-14/evidence/audit-pro-{before,after-cachebust}.*`, `metadata-registry-before.txt`, `validate-strict-before.txt`. Исправлен счётчик P2 (11→10) и total-open (48→46) — предыдущий подсчёт был off-by-one. CI AuditRepo валидатор PASS.
- **2026-07-14 — Аудиторский проход (arena-auditor-2026-07-14):** после 581 нового коммита в `origin/main` и 4 дней дрейфа — структурный ремонт AuditRepo и реверифация source-HEAD до `2ca2af3b`. Исправлены: корневой `DEBT-REGISTER.md` → `working/atlas/DEBT-REGISTER.md`; корневой `verification/atlas/` (27 PNG) → project-local `verification/atlas/root-evidence-2026-07-11/`; intake `claude-atlas-deep-audit/2026-07-10` доведён до контракта (README+REPORT); intake `milestone-atlas-v1` переименован в валидную папку `2026-07-14/`. CI валидатор вновь PASS. Добавлен новый P0 **DEPLOY-STATIC-GATES-RED-2026-07-11** (продакшн красный с 11 июля на Static publication gates); P2 **ATLAS-D-NAMESPACE-COLLISION** (атласовые D-16..19 пересекаются с историческими); P3 **AR-006** (валидатор не проверяет root directories). Создан `reverify/CURRENT_HEAD_REVERIFY_2026-07-14_2ca2af3b.md`.
