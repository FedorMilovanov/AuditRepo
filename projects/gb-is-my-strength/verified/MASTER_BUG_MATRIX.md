...
|---|---|---|
| `SEARCH-P3-02` | One truthful continuation contract across Pagefind, manifest fallback and exact Scripture occurrences: visible total/shown state + deterministic continuation, without reopening global shortcut ownership or leaving a self-writing merge transport. | Product `#1209` remains owner. Latest observed head `12896c2e40b4cb359ecadedb7bbe1f84c7b3cde2` contains current `main@11999f6d...` (`behind=0`). Temporary self-writing finalizer workflow/script are **absent from the current-main semantic diff**; the old writer-policy blocker is closed. Current scope is five semantic Search/test owners plus deterministic asset-revision projections. Current projection carries `command-palette.css@3b88813f` and `search.js@027c3f4f`; sampled PageHead/Chrome/Footer/Body consumers are hash-only and Spravochnik backstage metadata is unchanged. Exact-head Search Modal and Search Scripture Occurrence Runtime are now SUCCESS, as are Shared Files, Node Toolchain, Metadata, Scripture index/suggestion and multiple source/readiness gates; several other effective workflows remain non-terminal at the latest check. PR body is still stale (`1f14761a...` / `882d904...`, old semantic-owner list and predecessor references). Merge only after every effective exact-head workflow is terminal SUCCESS, remaining broad consumer touches are verified as deterministic projections, and the PR record is refreshed to the actual final SHA/scope/adjacent owners. [`REPORT`](../verification/2026-08-08-post-s12-manifest-parity-search-writer/REPORT.md) |
...
| `SYS-SOURCE-AUTHORITY-TRIGGER-CLOSURE` | Make Source Authority workflow applicability fail closed against the publication source surfaces its static-publication validation actually consumes. | Product issue `#1244` is the canonical guard-health owner. Merged `#1245` closed the concrete Baptist false-negative by adding `src/content/articles/**` and `src/components/baptisty-rossii/**` to PR/push filters, so that witness must not remain a direct defect. The full issue DoD remains open: derive closure from existing route/source authority rather than another ad-hoc list, prove representative protected source mutations make the workflow applicable for both PR and push, and mutation-test removal of coverage so a future validation-scope expansion cannot silently recreate the same false-negative class. Current `route-source-contract.js` already exposes route records plus traversed `inspection.files`, which is the preferred authority seed for a bounded closure census; do not replace the existing manual filter with an unreviewed catch-all merely to obtain green. [`REPORT`](../verification/2026-08-08-total-current-gold-audit/REPORT.md) |
...
- `#1209` owns Search continuation at observed head `12896c2e...`, `behind=0`. Temporary writer transport is gone; current semantic scope is Search/test owners plus deterministic hash projections. Search Modal and Scripture Runtime are now green along with Shared Files and multiple source/toolchain gates, while the overall exact-head suite remains non-terminal. PR body is stale and must be refreshed before merge even if all remaining CI becomes green.
...
- `#1224` remains SYSTEM authority for reader-control semantics. `#1240` owns the shared Back root (174 manifestations); `#1246` owns relation synchronization (64/70 missing `aria-controls`). Both remain two-file scopes but are `behind=2` and must refresh/rerun. Next bounded roots after collision-safe sequencing: conditional quiz panel, shared list-track semantics, Nagornaya relation/clipping, then repaired dynamic census. Footnotes stay under separate issue `#1225`.
- `#1212` remains audit-only all-reading-route control census. Its raw 887 manifestations are decomposition evidence, not Product repair. Static clusters are current-main applicable; **do not promote its 124 click failures, 207 size-prefilter observations or 12 runtime errors as direct defects without the additional isolation/spacing/environment evidence described above**. A branch named `agent/system-article-control-census-20260808-r2` exists but no separate open successor PR was verified at this refresh.
...
- `#1245` is **merged** as `11999f6d...`; Baptist MDX/body Source Authority trigger coverage is closed.
...
- Do not combine Search, S12 source/metadata, runtime asset identity, JS escaper consolidation, catalog projection, Current-Gold tooling, reader-controls, footnote projection, Source Authority trigger closure or Strangler work into one shared refactor wave.
