...
| Поле | Значение |
|---|---|
| Product verification anchor | `3a0f21b0ec01e423a2625becf13f600a07a6ddb5` |
...
|---|---|---|
| `CATALOG-PROJECTION-01` | Replace the hand-maintained `/articles/` membership/metadata owner with an exhaustive projection from existing publication/discovery authority, preserving truthful author/editor/translator roles. | Clean successor **#1348@ac48467d…** is current-main based and owns the catalog source/audits plus deterministic Scripture derivative. It is intentionally blocked on #1313 so the consumer cannot bake in the old editor-only role shape. This root also absorbs Lot human-orphan `55/56`; no one-off Lot card. Old #1221/#1305 are historical predecessors, not owners. |
| `SEARCH-MANIFEST-NEW-ROW-ROLE-AUTHORITY` | Make newly materialized Search rows preserve distinct author/editor/translator authority and never infer editor from meta-author. | Product issue **#1261**, draft PR **#1313@1611e7a…**. Regression-first proof already demonstrated old `author=undefined` / synthesized editor behavior; current semantic scope is the Search normalizer/workflow/test owner. After merge, #1348 and #1339 must absorb the new authority and rematerialize their derived projections canonically. |
...
| `SYS-READER-CONTROL-SEMANTICS` | Enforce truthful control→surface/action semantics across standalone and shared-series engines with one class-level browser census. | Product issue **#1224** remains root. Merged #1258/#1259 are retired slices. Current bounded residual **#1267@904d193…** gates `panelQuiz` with `tabQuiz` for `quiz: []`; it must refresh from current main before final acceptance. Audit-only **#1212@06c2b8e…** remains the all-reading-route census and is substantially stale by ancestry; it must stay diagnostic/fail-closed rather than weakening assertions to become green. |
...
| `SYS-STRANGLER-RETIREMENT` | Retire/quarantine retained legacy references only through the logical storage authority, with truthful blocker arithmetic and no physical move/delete before authorization. | Current Product main **#1362 / `3a0f21b…`** reports truthful readiness **13**. Draft **#1364@fe68234…** is the next bounded mechanical reader slice: make all six Gill claim legacy surfaces resolver-backed and fail closed at 6/6; expected effect **13 → 12** with reference/dependency/owner-decision counts otherwise stable. Physical legacy move/delete remains unauthorized. |
...
| `SYS-MAP-SCALE-RESIZE-WITNESS` | Make the Map scale resize browser witness measure settled runtime geometry rather than an intermediate CSS transition frame. | Draft **#1363@9f85b76…** is current-main based and changes only `scripts/map-engine-correctness-browser-test.mjs`. Runtime already recomputes from rendered canvas width; the old fixed 120ms sleep sampled a `.3s` width transition. New bounded convergence wait must preserve the same ≤2.5px invariant and fail if ResizeObserver/runtime actually stops updating. |
...
3. #1364 is one Strangler mechanical slice only; do not combine with Lot, reader or Search work.
...
No retired item remains as an active row. Details are preserved in the reconciliation report. In particular: `BAPT-S12-01` (#1260), Search existing-row reconciliation (#1254 / 46→0), Editorial pre-merge freeze (#1272/#1278), Gill Part I glossary residuals (#1283), reader slices #1258/#1259, old catalog predecessors #1221/#1305, the hidden Strangler self-verifier defect (#1270), and the Lot “quiz does not render” false positive are **not active MASTER work**.
