...
Current wave evidence: `verification/2026-08-08-search-ar-idx-09-closure/REPORT.md`.
...
| Wave | Search AR-IDX-09 closure + exact Strangler re-read, 2026-08-08 |
...
|---|---|---|
| `S-SEC-01` | `js/enhancements.js` всё ещё использует fixed blacklist/attribute-stripping sanitizer design для FAQ JSON-LD extraction. Текущая evidence доказывает design-hardening gap, но сама по себе не доказывает исполняемый XSS path. | Shared Search/cache owners уже merged; отдельная SYSTEM security-design lane теперь свободна. Начать с точного threat model и adversarial fixtures вокруг source → sanitizer → JSON-LD sink, а не с предположения exploitability. `verification/2026-08-08-search-ar-idx-09-closure/REPORT.md` |
| `NG-INLINE-01` | Одинаковый light-only блок `Из библиотеки` с hardcoded `#faf8f5`, `#b8882a`, `#8a5c10`, `#8a7968`/rgba presentation повторён в Nagornaya Parts **I, II, III и V**. Это один series-level presentation root, а не Part-I-only симптом. | Part IV block отсутствует. Part-I-only `#1186` закрыт unmerged как superseded. Следующий repair должен иметь одного scoped series owner для I/II/III/V, сохранить текст/ссылки/семантику и не тащить unrelated global CSS/JS. `verification/2026-08-07-live-release-strangler-nagornaya-reconciliation/REPORT.md` |
...
| `SEARCH-P3-02` | Add truthful result-total / continuation (`Показать ещё`, pagination or equivalent) instead of silently exposing only Pagefind 10 / fallback 12. | Current corpus can return more matches than the user can reach. Global shortcut ownership is now closed by Product #1183, so this remains an independent Search product improvement. `verification/2026-08-07-full-matrix-consolidation/REPORT.md` |
| `AR-IDX-05` | Consolidate Home/shared cache/version identity so `SITE_CONFIG.version` and asset `?v=` revisions do not remain parallel manual authorities. | Verified ownership debt with stale-cache/regression potential. #1187 and #1183 now establish the authority-aware cache/reference boundary that any future consolidation must preserve. `verification/2026-08-07-full-matrix-consolidation/REPORT.md` |
...
| `SYS-STRANGLER-RETIREMENT` | Current authority/identity/parity remains clear for the retained legacy/native-shadow surface: 53/53 public indexes covered, 53 ledger entries, 52/52 native shadows classification-clear, 0 unknown reference decisions, 0 integrity/inventory/parity problems. | Exact Product #1183 Shared Files artifact `9011117504` (`sha256:8b3ca435...`) now reports **35 dependency records / 26 blockers**: **16 mechanical repoints + 3 obsolete/remove-or-repoint readers + 7 dependency owner decisions**. `deletionReady=false`, `physicalMoveAuthorized=false`, verdict `NOT_YET_SAFE_TO_MOVE_OR_DELETE`. The increase from #1176’s 23 reflects explicit governed dependency readers added by #1187/Search, not reopened reference ambiguity. `verification/2026-08-08-search-ar-idx-09-closure/REPORT.md` |
...
- No open Product PR currently owns `gb-is-my-strength` after Product #1183 merged.
- `NG-INLINE-01` is therefore the cleanest scoped next Product defect lane: Parts I/II/III/V only, token/presentation repair, Part IV untouched, no unrelated shared runtime.
- `S-SEC-01` is also free of the former Search/cache collision, but should remain a separate SYSTEM security-design lane with threat-model/adversarial evidence before implementation.
- `SYS-STRANGLER-RETIREMENT` remains its own dependency/quarantine package; do not physically move/delete retained shadows while `blockerTotal=26`.

Recently merged/closed material current to this reconciliation:

- `#1183` — global Search keyboard ownership/current-runtime authority repair; exact candidate `853b99ca...` passed every triggered workflow group and merged as Product `67c23492...`; `AR-IDX-09` is retired by this wave;
- `#1194` — active-PR lane collision guard merged as Product `b9734532...` before final Search sync;
- `#1187` — authority-aware cache-bust/reference-only boundary merged as Product `b2720eee...`;
- `#1193` — repaired the Hermenevtika accepted-semantic anchor and wired existing Source Authority / Content Source Truth triggers; merged as Product `f3e291b7...`;
- canonical push-to-main Pages run `31215559649` on `f3e291b7...` produced generic and TTS live artifacts bound to the same release/run/candidate, both `result=PASS`, `phase=complete`; former `AUDIT-P2-WORKFLOWS-CHECK-GAP` remains retired;
