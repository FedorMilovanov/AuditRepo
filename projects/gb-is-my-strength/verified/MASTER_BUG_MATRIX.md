...
|---|---|---|
| `AUDIT-CSS-DEAD-KEYFRAMES-TOKENS` | Canonicalize the current duplicate `@keyframes fx-breathe` ownership in `css/site.css`; keep one intentional definition. | Fresh Product re-read after `#1197` confirms two different `fx-breathe` definitions. The older wording about a duplicate mobile `.gb-floater` visual rule was **not** reconfirmed and is no longer asserted. `verification/2026-08-08-direct-defects-zero/REPORT.md` |
| `AUDIT-JS-ESCAPER-DUP-X5` | Add one appropriate shared HTML-escaping primitive and migrate the real current local consumers instead of maintaining security-sensitive escaping independently. | Historical ID contains `X5`, but the count is no longer treated as a current invariant. Fresh re-read directly confirms separate local escapers in at least `js/search.js`, `js/site.js` and `js/highlights.js`; enumerate the current consumer set before implementation. `verification/2026-08-08-direct-defects-zero/REPORT.md` |
...
| `AR-IDX-05` | Consolidate cache/version identity so per-asset hash revisions and `SITE_CONFIG.version` do not remain parallel authorities for dynamic/static resources. | Fresh re-read confirms `src/lib/asset-version.js` owns per-asset hashes while runtime still consumes `SITE_CONFIG.version` for dynamic versioning such as glossary loading. `verification/2026-08-08-direct-defects-zero/REPORT.md` |
