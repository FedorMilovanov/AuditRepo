...
Historical source findings may be retired only after an exact current-source forensic check. A green umbrella deploy is supporting evidence, not a substitute for checking the named pattern, command or browser behavior when the historical claim is narrower.
