...
- На current source подтверждены активные engineering roots в community, theme/runtime accessibility, discovery metadata, semantic reader text, authoring ID contract, audit harness, audio session/release integrity, analytics consent/route state, rating-source authority и production legacy-route hosting. Их единственная рабочая очередь — [`verified/MASTER_BUG_MATRIX.md`](verified/MASTER_BUG_MATRIX.md). Текущий счётчик: **18 rows — 1 P1 + 16 P2 + 1 P3**.
...
- **Последний current-check:** [`verification/2026-08-12-release-hosting-discovery-current/REPORT.md`](verification/2026-08-12-release-hosting-discovery-current/REPORT.md)
