...
- Result:
  ...
  - `TLP-COMM-ABUSE-001` kept active and P1, but its row now carries a **freshly measured** reachability witness: the prior one was taken at anchor `d59ccec`, before #420/#422 moved that very boundary, so it was re-measured live instead of re-quoted;
...
- Current-source verification: `FedorMilovanov/TheLegendaryPoet@main` `57353dcee63123e8e2a86fa83bc964ffa5f29303`. The three closed roots' merges are all ancestors of that head, and their permanent guards `qa/theme-contrast.spec.mjs` and `qa/reader-text.spec.mjs` are present. **All 21 active rows** were re-checked directly against that head with file/line witnesses and all 21 remain current — 0 fixed-current, 0 stale, 0 regressions; nothing merged after PR #427 touches an active row (`#428`/`#429` are stale-ref maintenance).
