...
**Latest current verification:** `../reverify/REVERIFY_PREMERGE_2026-09-08_active-row-currency.md`.  
**Closure history:** `CLOSURE_LEDGER.md`.  
**Latest integrity audit:** `../verification/2026-09-06-ssot-matrix-integrity-audit/REPORT.md`.  
**Latest full-row currency check:** `../reverify/REVERIFY_PREMERGE_2026-09-08_active-row-currency.md` — all 18 rows were rechecked against current Product source before Hall #466 terminal merge; `TLP-RATING-URLSTATE-001` and `TLP-AUDIO-COMPLETION-001` are conclusively closed by already-merged Product #438/#439 and are removed below. The remaining 16 roots are still current or externally gated; the report remains explicitly pre-merge for Hall #466 provenance until the resulting Product `main` exists.
...
| `TLP-READING-PROGRESS-001` | `CONFIRMED-CURRENT / READER-SEMANTICS / PROGRESS-BOUNDARY / P3` | `ReadingProgress` still uses `animation-timeline: scroll(root)` in the CSS path and `document.documentElement.scrollHeight` in the JS path. `EssayPage` already owns an explicit `articleRef`, but progress ignores it, so post-article community/footer tail extends the reading denominator. | Own progress by explicit article boundaries and prove 100% at article end despite long post-article tail. |
...
| `TLP-A11Y-MOTION-001` | `CONFIRMED-CURRENT / ACCESSIBILITY / REDUCED-MOTION / P3` | Framer/Tilt/View Transitions honor reduced motion, but utility CSS animations do not: PoetCard has perpetual decorative `animate-pulse` and audio surfaces use unguarded `animate-pulse`/`animate-spin`. Product #467 now owns the bounded source repair after Hall #466 is terminal. | One motion-policy contract across Framer/JS/CSS utilities; suppress non-essential persistent motion under reduce, preserve state meaning and add computed-animation regressions. |
...
| Рефакторинг | 0 |
| AuditRepo | 0 |
| **Всего открыто (матрица)** | **16** |

Current architecture selection: **none**. Historical Hall #369 remains terminally closed; later Hall work is owned only by bounded follow-up Product transactions such as current #465/#466, not by reopening the old architecture root.
