...
script_path = Path(__file__).with_name("_temp_reconcile_gb_production_witness.py")
source = script_path.read_text(encoding="utf-8")
start = source.index("REVERIFY.write_text(")
end = source.index('\n\nprint("reconciled production witness SSOT")', start)
replacement = '''REVERIFY.write_text(
...
compile(fixed, str(script_path), "exec")
