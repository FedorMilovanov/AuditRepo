#!/usr/bin/env python3
"""Scaffold a retirement-review file for a suspected stale/false-positive finding.
...
typo or hostile value can never write outside the project's verification tree.
"""
...
def main() -> int:
    ...
    if not safe_component(args.project, 'project'):
        return 1
    if not safe_component(args.bug_id, 'bug_id'):
        return 1
    ...
    if out.exists():
        print(f'ERROR: retirement review already exists and will not be overwritten: {out}', file=sys.stderr)
        return 1
